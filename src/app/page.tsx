...
useEffect(() => {
  let aparecer: NodeJS.Timeout | undefined
  let sumir: NodeJS.Timeout | undefined
  ...
  const ReproduzirTexto = (textoAudio: string) => {
   if(audio) {
      const mensagem = textoAudio.replace(/[\u{1F300}-\u{1F9FF}|\u{2600}-\u{26FF}\u{2700}-\u{27BF}]/gu, '').trim()
  ...
  return (
   <div ref={chatref} className={`h-svh flex flex-col ${rolagem? 'overflow-y-auto scroll-hidden' : ''}`}>
    ...
    {Cardativo && (
      <Ativo className={`transition-all duration-1000 ease-out ${Remove? 'opacity-100 scale-100 translate-y-0':'opacity-0 scale-80 translate-y-5 pointer-events-none'}`}></Ativo>
    )}
    {desativado && (
      <Desativado className={`transition-all duration-1000 ease-out ${Remove? 'opacity-100 scale-100 translate-y-0':'opacity-0 scale-80 translate-y-5 pointer-events-none'}`}></Desativado>
    )}

    {card && (
      <div className={`absolute font-sans lg:left-110 lg:bottom-10 bg-slate-900 rounded-2xl left-3 bottom-4 w-70 transition-opacity duration-700 p-8 z-50 ${transion? 'opacity-100': 'opacity-0'}`}>
